...
To begin the analysis, we can see that we are working with a good sample of the population, since we have users ranging from not at all active to fully active users, and there are also several intermediate users, which favors an unbiased analysis.
```{r}
ggplot(df_user_activity, aes(x = user_activity_in_period)) +
  geom_histogram(binwidth = 10, fill = "#00BFC4", color = "black", alpha = 0.7) +
  labs(title = "Distribuição do Nível de Atividade dos Usuários (%)",
       x = "% de Dias Ativos",
...
A maioria dos usuários de dispositivos inteligentes é “Lightly Active” em todos os dias da semana.

Poucos minutos são dedicados a atividades moderadas ou intensas.

Estratégias de produto e marketing devem focar em apoiar e estimular a rotina ativa do dia a dia, não só o desempenho esportivo.
